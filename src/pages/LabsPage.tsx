...
export function LabsPanel({ locale = 'en' }: { locale?: NativeLocale }) {
  ...
  return <section className="space-y-5" aria-labelledby="labs-title">
    ...
    <div className="grid gap-5 xl:grid-cols-[18rem_minmax(0,1fr)]"><aside className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><h3 className="px-2 pb-2 text-sm font-semibold capitalize">{difficulty} experiments</h3><div className="space-y-2">{visiblePresets.map((preset) => <button key={preset.id} type="button" className={activePreset.id === preset.id ? 'lab-preset native-tab native-tab-active w-full text-left' : 'lab-preset native-tab w-full text-left'} onClick={() => choosePreset(preset)}><span className="block font-semibold">{preset.title}</span><span className="text-xs opacity-70">{preset.operation}</span></button>)}</div></aside>
