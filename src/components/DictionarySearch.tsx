...
export default function DictionarySearch({ showQuickCategories = false }: DictionarySearchProps) {
  ...
  return <div>
    ...
    {results.length ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-500"><p>Showing {(currentPage - 1) * pageSize + 1}-{Math.min(currentPage * pageSize, results.length)} of {results.length} entries</p><p>Page {currentPage} / {totalPages}</p></div> : null}
