...
export default function DictionarySearch({ showQuickCategories = false }: DictionarySearchProps) {
  ...
  return <div>
    ...
    {showQuickCategories ? <div className="mt-3 flex flex-wrap gap-2">{quickFilters.map((category) => <button key={category} type="button" className={selectedCategory === category ? 'native-tab native-tab-active' : 'native-tab'} onClick={() => setSelectedCategory(category)}>{category}</button>)}</div> : null}
    ...
    <div className="mt-5 grid gap-4 md:grid-cols-2">{paginatedResults.map((entry) => <article key={entry.id} className="native-card"><div className="flex justify-between gap-3"><div><h2 className="text-xl font-semibold">{entry.name}</h2>{entry.symbol ? <p className="mt-1 text-sm text-slate-500">Symbol: <code className="native-code">{entry.symbol}</code></p> : null}</div><span className="native-pill">{entry.category}</span></div><code className="native-code mt-4 inline-block">{entry.code}</code><p className="mt-4 text-slate-600 dark:text-slate-300">{entry.description}</p><p className="mt-3 text-sm text-slate-500">Usage: {entry.usage}</p></article>)}</div>
