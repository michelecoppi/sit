...
export default function AlphabetTable() {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap gap-3"><input className="native-input" value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Search symbols, meanings, or codes" />
      <div className="flex flex-wrap gap-2">{['All', ...nativeCategories].map((item) => <button key={item} type="button" className={category === item ? 'native-tab native-tab-active' : 'native-tab'} onClick={() => setCategory(item)}>{item}</button>)}</div></div>
